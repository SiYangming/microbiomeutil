...
my $low_range_finer_BS = 10;
my $num_finer_BS_replicates = 1000;
...
main: {
	...
	for (my $i = 0; $i < $#reference_alignments; $i++) {
		...
		for (my $j = $i+1; $j <= $#reference_alignments; $j++) {
			...
			foreach my $div_struct (@div_structs) {
				...
				$div_struct = &bootstrap_divstruct($div_struct, $NUM_BS_REPLICATES);
	...
	if (@all_entries) {
		...
		foreach my $div_struct (@all_entries) {
			...
			my $max_BS = max( $div_struct->{BS_A}, $div_struct->{BS_B});
			
			if ($max_BS >= $minBS - $low_range_finer_BS) {  ## BS values are close enough to implicate, but it could be that not enough BS replicates were originally performed (since they're expensive).
				$div_struct = &bootstrap_divstruct($div_struct, $num_finer_BS_replicates);
			}
...
sub report_findings {
	...
	my $SNPs_left_aref = $data_struct->{SNPS_LEFT};
	my $SNPs_right_aref = $data_struct->{SNPS_RIGHT};
	...
	print "\n\nSNPs used for BS calculations:\n\n";
	&print_SNP_sets($SNPs_left_aref, $SNPs_right_aref);
...
sub bootstrap_SNPs {
	my ($snpsLeft_aref, $snpsRight_aref, $num_bs_replicates) = @_;
...
####
sub bootstrap_divstruct {
	my ($div_struct, $num_bs_replicates) = @_;
	...
	my ($BS_A, $BS_B) = &bootstrap_SNPs(\@snpsLeft, \@snpsRight, $num_bs_replicates);
	...
	$div_struct->{SNPS_LEFT} = \@snpsLeft;
	$div_struct->{SNPS_RIGHT} = \@snpsRight;
