...
my $IGNORE_IDENTICAL_PARENT_SEQ_FLAG = 0;
...
&GetOptions ( 'h' => \$help_flag,
			  ...
			  ## testing purposes
			  'ignore_identical_parent_seq' => \$IGNORE_IDENTICAL_PARENT_SEQ_FLAG,
...
main: {
	...
	@compareMalignments = &apply_min_query_coverage_filter($queryAlignStruct, \@compareMalignments, $minQueryCoverage); # also, ignore parent sequences that are identical to query if IGNORE_IDENTICAL_PARENT_SEQ_FLAG is set (testing purposes).
...
sub compute_percent_query_aligned_bases {
    ...
    #print STDERR "-checking for identity.\n";
    if ($IGNORE_IDENTICAL_PARENT_SEQ_FLAG && &are_identical_seqs($query_alignment->{align}, $other_alignment->{align})) {
        ## avoid effectively self-comparisons
        print STDERR "warning, query " . $query_alignment->{acc} . " and parent sequence " . $other_alignment->{acc} . " are identical. Ignoring this parent.\n";
		return(0);
    }
...
####
sub are_identical_seqs {
    my ($seq_A_align, $seq_B_align) = @_;
    
    my $seq_A = &get_seq_from_align($seq_A_align);
    my $seq_B = &get_seq_from_align($seq_B_align);
...
####
sub get_seq_from_align {
    my ($align_aref) = @_;

    my @chars;

    foreach my $ele (@$align_aref) {
        my $char = $ele->{char};
        push (@chars, $char);
    }
