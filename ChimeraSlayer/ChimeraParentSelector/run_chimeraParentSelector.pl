...
main: {
	...
	while (my $seq_obj = $fasta_reader->next()) {
		...
		eval {
			...
			if ($IGNORE_IDENTICAL_PARENT_SEQ) {
				$cmd .= " --ignore_identical_parent_seq ";
			}
...
sub blast_seq {
	...
	foreach my $result (@results) {
		...
		if ($IGNORE_PARENT_SAME_ACC_SUBSTRING && $x[0] =~ /^$x[1]\W/) {
			print STDERR "warning: candidate parent $x[1] is a substring of the query: $x[0] and so being ignored.\n"; 
			next;
		}
